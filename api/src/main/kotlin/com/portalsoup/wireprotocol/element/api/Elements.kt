...
fun WireProtocol.findElement(session: SessionCreated, using: LocationStrategy): Response =
    post("/session/${session.id}/element", FindElementStrategy(using.id.code, using.expression))

fun WireProtocol.findElements(session: SessionCreated, using: LocationStrategy): Response =
    post("/session/${session.id}/elements", FindElementStrategy(using.id.code, using.expression))
...
fun WireProtocol.findElementFromElement(session: SessionCreated, locationStrategy: LocationStrategy, parent: ElementRef): Response =
    post("/session/${session.id}/element/${parent.reference}/element", locationStrategy.expression)
...
fun WireProtocol.findElementsFromElement(session: SessionCreated, locationStrategy: LocationStrategy, parent: ElementRef): Response =
    post("/session/${session.id}/element/${parent.reference}/elements", locationStrategy.expression)

fun WireProtocol.findElementFromShadowRoot(session: SessionCreated, rootReference: String, using: LocationStrategy): Response =
    post("/session/${session.id}/shadow/${rootReference}/element", FindElementStrategy(using.id.code, using.expression))

fun WireProtocol.findElementsFromShadowRoot(session: SessionCreated, using: LocationStrategy, shadowId: String): Response =
    post("/session/${session.id}/shadow/${shadowId}/elements", FindElementStrategy(using.id.code, using.expression))
